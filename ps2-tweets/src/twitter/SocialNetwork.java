...
public class SocialNetwork {
	...
	public static Map<String, Set<String>> guessFollowsGraph(List<Tweet> tweets) {
		...
		for (Tweet tweet : tweets) {
			...
			Set<String> mentionsInTweet = Extract.getMentionedUsers(Arrays.asList(tweet));
			
			mentionsInTweet.remove(user);
			
			if (!mentionsInTweet.isEmpty()) {
				if (followGraph.get(user) == null) {
					followGraph.put(user, mentionsInTweet);
				} else {
					followGraph.get(user).addAll(mentionsInTweet);
				}
			}
